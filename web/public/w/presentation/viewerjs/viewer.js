...
function loadPlugin( pluginName, callback ) {
    ...
    script.src    = 'viewerjs/'+pluginName + '.js';
...
function Viewer( viewerPlugin, parameters ) {
    ...
    this.initialize = function () {
        ...
        viewerPlugin.onLoad = function () {
            ...
            self.actuallyShowPage(readStartPageParameter(parameters.startpage));
    ...
    this.showPage = function ( n ) {
        ...
        window.widgetObj.showPage(n)
    };

    this.actuallyShowPage = function ( n ) {
        if ( n <= 0 ) {
            n = 1;
        } else if ( n > pages.length ) {
            n = pages.length;
...
(function () {
    ...
    window.onload = function () {
        ...
        if ( !parameters.title ) {
            parameters.title = parameters.documentUrl.replace(/^.*[\\\/]/, '');
        }
        
        // trust the server most
        estimateTypeByHeaderContentType(parameters.documentUrl, function ( pluginData ) {
            if ( !pluginData ) {
                if ( parameters.type ) {
                    pluginData = estimateTypeByFileExtension(parameters.type);
                } else {
                    // last ressort: try to guess from path
                    pluginData = estimateTypeByFileExtensionFromPath(parameters.documentUrl);
                }
            }
        ...
        let curInControlDomState = 'hidden'
        let controllerId = ''
        setInterval(() => {
            if(!viewer) return
            const widgetObj  = window.widgetObj || {}
            if(viewer.currentPage !== widgetObj.curPage) {
                viewer.actuallyShowPage(widgetObj.curPage);
                viewer.parseScale('page-fit')
            }
            if(!widgetObj.inControl) {
                if(curInControlDomState != 'hidden') {
                    document.getElementById('toolbarLeft').style.visibility = 'hidden'
                    document.getElementById('toolbarRight').style.visibility = 'visible'
                    curInControlDomState = 'hidden'
                }
                if(controllerId != widgetObj.inControlId) {
                    document.getElementById('controllerId').innerText = widgetObj.inControlId 
                    controllerId = widgetObj.inControlId
                }
            }
            if(widgetObj.inControl && curInControlDomState != 'visible') {
                document.getElementById('toolbarLeft').style.visibility = 'visible'
                document.getElementById('toolbarRight').style.visibility = 'hidden'
                curInControlDomState = 'visible'
            }
